...
// One-time scoped removal (2026-07-20): 3 specific transactions on the
// Discount cashname "103-200-4547" — purchased 2026-07-07 but stuck with
// billing_date 2026-07-02 (a purchase can't be debited before it happened;
// the repair above didn't change them, meaning the bank's own raw DebitDate
// genuinely reads 2026-07-02 for these rows, not just stale pre-fix data) —
// were blocking that page's push (real bank debit ₪6,602.24 vs our summed
...
// Matched by bank_id + exact date+amount (not priority_cashname, which is a
// free-typed field that could differ by whitespace/dash and silently match
// nothing — and not merchant text, which the UI truncates), so this can
// never touch any other row. Idempotent — matches nothing once already
// deleted.
{
  const res = db.prepare(`
    DELETE FROM card_transactions
    WHERE card_id IN (SELECT id FROM credit_cards WHERE bank_id = 'discount')
      AND purchase_date = '2026-07-07'
      AND billing_date = '2026-07-02'
      AND amount IN (-80, -55, -290)
  `).run();
  if (res.changes > 0) {
    console.log(`[credit-cards] removed ${res.changes} stuck transaction(s) (2026-07-02 mismatch, cashname 103-200-4547)`);
  }
}
